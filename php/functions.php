...
function createForm( $form = NULL ){
    ...
    if ( $form ) {
        ...
        if ( $form->errorBox != "" && $form->errorBoxPos == 1 ) { 
...
function replaceValues( $value = NULL ) {
    ...
    $arrayReplace = array(date('Y'), date('Y-m-d'),date('Y-m-d', strtotime("+ 1 day")),'<a href="#" target="_blank">'.$config->info->titulo.'</a>','<a href="http://www.adview.mx" target="_blank" aria-label="Adview"><img src="images/logo-adview.png" alt="Adview" class="img-fluid" /></a>');
...
function getMainCSS() {
    ...
    if ( $config->configuracion->advanced->maincss != "" ) {
        $maincss = explode(",",$config->configuracion->advanced->maincss);
    
        foreach( $maincss as $filecss ): 
            $eachCSS .= '<link rel="stylesheet" media="screen" href="'.$filecss.'">'."\n";
        endforeach;

    } else {

        $eachCSS .= '<link rel="stylesheet" media="screen" href="dist/css/styles.css">'."\n";
...
function getMainJS() {
    ...
    if ( $config->configuracion->advanced->mainjs != "" ) {
        ...
        $eachJS .= '<script async src="dist/js/bundle.js"></script>'."\n";
